...
def run_exp2(rl_method1, rl_method2, policy1, policy2, env, world_state, q_table, m_agent, f_agent, alpha, gamma):
    ...
    for i in range(10000):
        ...
        q_table.update_qtable(world_state.representation['male_position']['cell_type'], chosen_action, layer,
                              cells[chosen_action]['type'], next_pos_actions, alpha, gamma, chosen_rl_method,
                              chosen_policy, future_carrying, next_pos_cells)
            ...
            for cell in world_state.representation['dropoff_cell_blocks']:
                if world_state.representation['dropoff_cell_blocks'][cell] == 5:
                    ...
                    for layer in ['carrying', 'not_carrying']:
                        print(layer)
                        print('{:^10}'.format(' '), '{:^15}'.format('up'), '{:^15}'.format('down'),
                              '{:^15}'.format('left'), '{:^15}'.format('right'), '{:^15}'.format('forward'),
                              '{:^15}'.format('backward'))
